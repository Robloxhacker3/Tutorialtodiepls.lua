...
local Window = Library:CreateWindow("FRXZN👍 Evade", Vector2.new(500, 300), Enum.KeyCode.RightShift)
local Evade = Window:CreateTab("General")
local AutoFarms = Window:CreateTab("wow")
...
local WalkSpeed = EvadeSector:AddSlider("Speed", 1450, 1450, 80000, 100, function(Value)
    Settings.Speed = Value
end)


local JumpPower = EvadeSector:AddSlider("JumpPower", 3, 3, 80, 1, function(Value)
    Settings.Jump = Value
end)
...
Gamesec:AddToggle('Fast Revive', false, function(State)
    if State then
        workspace.Game.Settings:SetAttribute("ReviveTime", 0.2)
    else
...
Credits:AddLabel("Developed By mr.banana And scarlet")
Credits:AddLabel("UI BANANA")
Credits:AddLabel("ESP Lib: banana")
